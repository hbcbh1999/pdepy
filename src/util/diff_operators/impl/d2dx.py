...
class d2dx(diff_op.diff_operator):
    ...
    def getIrregularStencil(self, dx, tau, direction):
        ...
        if direction == diff_op.Direction.POSITIVE:
            return [irregular_stencil_node((-1, 0), c1/dx**2), irregular_stencil_node((0, 0), c2/dx**2), irregular_stencil_node((tau, 0), c3/dx**2, True)]
        elif direction == diff_op.Direction.NEGATIVE:
            return [irregular_stencil_node((-tau, 0), c3/dx**2, True), irregular_stencil_node((0, 0), c2/dx**2), irregular_stencil_node((1, 0), c1/dx**2)]